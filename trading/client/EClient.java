...
public abstract class EClient {
    ...
    public static final int MAX_VERSION = MIN_SERVER_VER_PENDING_PRICE_REVISION; // ditto
    ...
    public synchronized void reqMktData(int tickerId, Contract contract,
        ...
        try {
            ...
            if (m_serverVersion >= 8 && Types.SecType.BAG.name().equalsIgnoreCase(contract.getSecType())) {
    ...
    public synchronized void reqHistoricalData(int tickerId, Contract contract,
        ...
        try {
            ...
            if (m_serverVersion < MIN_SERVER_VER_HISTORICAL_SCHEDULE) {
                if (!IsEmpty(whatToShow) && whatToShow.equalsIgnoreCase(Types.WhatToShow.SCHEDULE.name())) {
                    error(tickerId, EClientErrors.UPDATE_TWS,
    ...
    public synchronized void placeOrder(int id, Contract contract, Order order) {
        ...
        if (m_serverVersion < MIN_SERVER_VER_ORDER_COMBO_LEGS_PRICE && Types.SecType.BAG.name().equalsIgnoreCase(contract.getSecType())) {
        ...
        try {
            ...
            if (m_serverVersion >= MIN_SERVER_VER_SMART_COMBO_ROUTING_PARAMS && Types.SecType.BAG.name().equalsIgnoreCase(contract.getSecType())) {
