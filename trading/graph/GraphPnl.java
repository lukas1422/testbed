...
//    private static final Predicate<? super Entry<LocalTime, ? extends Number>> TRADING_PRED = e
...
//                .collect(Collectors.toMap(Entry::getKey, Entry::getValue, (u, v) -> u, ConcurrentSkipListMap::new))
...
//                ? Math.round(100d * Stream.of(mtmMap, tradeMap, netMap).flatMap(m -> m.entrySet().stream()).min(Entry.comparingByValue()).map(Entry::getValue).orElse(0.0)) / 100d
//                : Math.round(100d * Stream.of(mtmMap, buyMap, sellMap, netMap).flatMap(m -> m.entrySet().stream()).min(Entry.comparingByValue()).map(Entry::getValue).orElse(0.0)) / 100d;
//    }
//
//    private double getMax() {
//        return (ChinaPosition.buySellTogether)
//                ? Math.round(100d * Stream.of(mtmMap, tradeMap, netMap).flatMap(m -> m.entrySet().stream()).max(Entry.comparingByValue()).map(Entry::getValue).orElse(0.0)) / 100d
//                : Math.round(100d * Stream.of(mtmMap, buyMap, sellMap, netMap).flatMap(m -> m.entrySet().stream()).max(Entry.comparingByValue()).map(Entry::getValue).orElse(0.0)) / 100d;
//    }
//
//    private double getMinDelta() {
//        return Math.round(100d * netDeltaMap.entrySet().stream().min(Entry.comparingByValue()).map(Entry::getValue).orElse(0.0)) / 100d;
//    }
//
//    private double getMaxDelta() {
//        return Math.round(100d * netDeltaMap.entrySet().stream().max(Entry.comparingByValue()).map(Entry::getValue).orElse(0.0)) / 100d;
//    }
//
//    private LocalTime getNetPnlMaxTime() {
//        return netMap.entrySet().stream().max(Entry.comparingByValue()).map(Entry::getKey).orElse(LocalTime.MAX);
//    }
//
//    private LocalTime getNetPnlMinTime() {
//        return netMap.entrySet().stream().min(Entry.comparingByValue()).map(Entry::getKey).orElse(LocalTime.MAX);
//    }
//
//    private double getMinAM() {
//        return (tm.size() > 0 && tm.firstKey().isBefore(LocalTime.of(12, 0)))
//                ? Math.round(100d * tm.entrySet().stream().filter(AMPRED).min(Entry.comparingByValue()).map(Entry::getValue).orElse(0.0)) / 100d : 0.0;
//        //.get().getValue()) / 100d : 0.0;
//    }
//
//    private double getMaxAM() {
//        return (tm.size() > 0 && tm.firstKey().isBefore(AMCLOSET)) ?
//                Math.round(100d * tm.entrySet().stream().filter(AMPRED).max(Entry.comparingByValue()).map(Entry::getValue).orElse(0.0)) / 100d : 0.0;
//    }
//
//    private double getMinPM() {
//        return (tm.size() > 0 && tm.lastKey().isAfter(PMOPENT)) ?
//                Math.round(100d * tm.entrySet().stream().filter(PMPRED).min(Entry.comparingByValue()).map(Entry::getValue).orElse(0.0)) / 100d : 0.0;
//    }
//
//    private double getMaxPM() {
//        return (tm.size() > 0 && tm.lastKey().isAfter(PMOPENT)) ?
//                Math.round(100d * tm.entrySet().stream().filter(PMPRED).max(Entry.comparingByValue()).map(Entry::getValue).orElse(0.0)) / 100d : 0.0;
//    }
//
...
//        if (tm.size() > 0) {
//            double maxD = tm.entrySet().stream().max(Entry.comparingByValue()).map(Entry::getValue).orElse(0.0);
//            double minD = tm.entrySet().stream().min(Entry.comparingByValue()).map(Entry::getValue).orElse(0.0);
