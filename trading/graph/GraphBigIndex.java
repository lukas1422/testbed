...
//    static final Entry<LocalTime, SimpleBar> dummyBar =  new AbstractMap.SimpleEntry<>(LocalTime.of(23,59), new SimpleBar(0.0));
    private static final Comparator<? super Entry<LocalTime, Double>> GREATER = Comparator.comparingDouble(Entry::getValue);
...
//            
    ...
    private double getMax() {
        double maxYtd = (tmYtd != null && tmYtd.size() > 0) ? tmYtd.entrySet().stream().filter(entry -> !entry.getValue().containsZero())
                .max(Utility.BAR_HIGH).map(Entry::getValue).map(SimpleBar::getHigh).orElse(0.0) : 0.0;
    ...
    private double getMinVol() {
        return Math.min(tmVolYtd.entrySet().stream().filter(entry -> entry.getValue() != 0L).min(Entry.comparingByValue()).map(Entry::getValue).orElse(0.0),
                tmVol.entrySet().stream().filter(entry -> entry.getValue() != 0L).min(Entry.comparingByValue()).map(Entry::getValue).orElse(0.0));
    }

    private double getMaxVol() {
        return Math.max(tmVolYtd.entrySet().stream().filter(entry -> entry.getValue() != 0L).max(GREATER).map(Entry::getValue).orElse(0.0),
                tmVol.entrySet().stream().filter(entry -> entry.getValue() != 0L).max(GREATER).map(Entry::getValue).orElse(0.0));
    }
        ...
        if (!tmYtd.isEmpty() & tmYtd.size() > 2) {
            if (tmYtd.firstKey().isBefore(LocalTime.of(12, 1)) && tmYtd.lastKey().isAfter(LocalTime.of(9, 30))) {
                return tmYtd.entrySet().stream().filter(entry -> !entry.getValue().containsZero())
                        .filter(Utility.AM_PRED).max(Utility.BAR_HIGH).map(Entry::getKey).orElse(LocalTime.of(9, 30));
            }
