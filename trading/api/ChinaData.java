...
//            AllData.priceMapBar.put(v, new ConcurrentSkipListMap<>());
//            AllData.priceMapBarDetail.put(v, new ConcurrentSkipListMap<>());
//            ytdData.put(v, new ConcurrentSkipListMap<>());
//            AllData.priceMapBarYtd.put(v, new ConcurrentSkipListMap<>());
...
//                oos.writeObject(AllData.priceMapBarYtd);
//            } catch (IOException e) {
//                e.printStackTrace();
//            }
...
//                AllData.priceMapBar = (ConcurrentHashMap<String, ConcurrentSkipListMap<LocalTime, SimpleBar>>) ois.readObject();
...
//            AllData.priceMapBar.replaceAll((k, v) -> new ConcurrentSkipListMap<>());
//            SwingUtilities.invokeLater(() -> m_model.fireTableDataChanged());
//            pr(" unloaded pmb ",
//                    AllData.priceMapBar.entrySet().stream().mapToInt(e -> e.getValue().size()).max().orElse(0));
//        });
//
//        unloadHibDetailButton.addActionListener(al -> {
//            AllData.priceMapBarDetail.replaceAll((k, v) -> new ConcurrentSkipListMap<>());
//            SwingUtilities.invokeLater(() -> m_model.fireTableDataChanged());
//            pr(" unloading detailed ",
//                    AllData.priceMapBarDetail.entrySet().stream().mapToInt(e -> e.getValue().size()).max().orElse(0));
//        });
...
//                //noinspection unchecked
//                AllData.priceMapBarYtd = (ConcurrentHashMap<String, ConcurrentSkipListMap<LocalTime, SimpleBar>>) ois.readObject();
//            } catch (IOException | ClassNotFoundException e3) {
//                e3.printStackTrace();
//            }
//        }, es).
...
//            getHistoricalCustom(controller(), AllData.GLOBAL_REQ_ID.addAndGet(5),
...
//            TradingUtility.getHistoricalCustom(controller(), AllData.GLOBAL_REQ_ID.addAndGet(5),
...
//            AllData.priceMapBarDetail.keySet().forEach(k -> {
//                if (currencyMap.getOrDefault(k, CNY).equals(HKD)) {
//                    Contract c = symbolToIBContract(k);
//                    TradingUtility.getHistoricalCustom(controller(), AllData.GLOBAL_REQ_ID.addAndGet(5), c
...
//                    TradingUtility.reqHistDayData(controller(), AllData.GLOBAL_REQ_ID.addAndGet(5),
...
//                try {
//                    out.append(Utility.getStrComma(s, AllData.priceMap.getOrDefault(s, 0.0), closeMap.getOrDefault(s, 0.0)));
//                    out.newLine();
...
//        CompletableFuture.runAsync(() -> Utility.getFilesFromTDXGen(dat, AllData.priceMapBar, ChinaData.sizeTotalMap));
//    }
//
//    private static void getFromTDX(LocalDate today, LocalDate ytd, LocalDate y2) {
//        CompletableFuture.runAsync(() -> {
//            Utility.getFilesFromTDXGen(today, AllData.priceMapBar, ChinaData.sizeTotalMap);
//            Utility.getFilesFromTDXGen(ytd, AllData.priceMapBarYtd, ChinaData.sizeTotalMapYtd);
...
//                ConcurrentSkipListMap<LocalTime, SimpleBar> nm = AllData.priceMapBar.get(ticker);
...
//                    .mapToDouble(Entry::getValue).average().orElse(0.0);
//
...
//        String ticker = Utility.ibContractToSymbol(c);
//
//        if (!date.startsWith("finished")) {
//            Date dt = new Date(Long.parseLong(date) * 1000);
//            Calendar cal = Calendar.getInstance();
...
//                double previousVol = Optional.ofNullable(ChinaData.sizeTotalMapYtd.get(ticker).lowerEntry(lt))
//                        .map(Entry::getValue).orElse(0.0);
//                AllData.priceMapBar.get(ticker).put(lt, new SimpleBar(open, high, low, close));
//                ChinaData.sizeTotalMap.get(ticker).put(lt, volume * 1d + previousVol);
//            }
//        } else {
...
//                AllData.priceMapBarDetail.get(symbol).put(ldt, open);
//            }
//        } else {
//            System.out.println(str(date, open, high, low, close));
//        }
...
//                            return (AllData.priceMapBar.get(name).containsKey(tradeTime.get(col - 2))) ? AllData.priceMapBar.get(name).get(tradeTime.get(col - 2)).getClose() : 0.0;
//                        }
//                    } catch (Exception ex) {
//                        System.out.println(" name in china map " + name);
//                        System.out.println(" priceMapBar " + AllData.priceMapBar.get(name));
//                        ex.printStackTrace();
//                    }
//                    return null;
//            }
