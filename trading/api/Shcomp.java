...
//                    minT = shcompPrice.entrySet().stream().filter(tradingRange).min((entry1, entry2) -> entry1.getValue() >= entry2.getValue() ? 1 : -1).map(Entry::getKey).get();
//                } catch (Exception ex) {
//                    ex.printStackTrace();
//                }
//            } else {
...
//                    .filter(a -> Double.parseDouble(a.getValue()) > max1).min(Entry.comparingByKey()).get().getKey() - 1;
//
//            minKey = IntStream.range(0, list.get(0).size()).boxed().collect(toMap(Function.identity(), i -> list.get(0).get(i))).entrySet().stream()
//                    .filter(a -> Double.parseDouble(a.getValue()) > min1).min(Entry.comparingByKey()).get().getKey() - 1;
//
//            currKey = IntStream.range(0, list.get(0).size()).boxed().collect(toMap(Function.identity(), i -> list.get(0).get(i))).entrySet().stream()
//                    .filter(a -> Double.parseDouble(a.getValue()) > curr1).min(Entry.comparingByKey()).get().getKey() - 1;
//
//            //list.get(1).
//            //       //minSum
