...
//import static enums.FXCurrency.USD;
...
//
//    private static final LocalDate LAST_MONTH_LAST_DAY = getMonthBeginMinus1Day();
//    private static final LocalDate LAST_YEAR_LAST_DAY = getYearBeginMinus1Day();
//
...
//    public static Map<FXCurrency, Double> fxMap = new HashMap<>();
...
//
//    private static final Predicate<Entry<String, ?>> CHINA_STOCK_PRED = m -> TradingUtility.isChinaStock(m.getKey());
//    private static final Predicate<Entry<String, ?>> FUT_PRED = m -> m.getKey().startsWith("SGXA50");
//    private static final Predicate<Entry<String, ?>> HK_PRED = e -> TradingUtility.isHKStock(e.getKey());
//
//    //private static volatile Predicate<Map.Entry<String, ?>> GEN_MTM_PRED = CHINA_STOCK_PRED.or(FUT_PRED);
//    private static volatile Predicate<Entry<String, ?>> GEN_MTM_PRED = e -> true;
...
//                                                .getPnl(selected, AllData.priceMap.getOrDefault(selected, 0.0)))),
//                                        e -> e.entrySet().stream().sorted(reverseComp(Comparator.comparing(Entry::getValue)))
...
//
//                            double last;
//                            if (AllData.priceMapBar.containsKey(symbol) && AllData.priceMapBar.get(symbol).size() > 0) {
//                                last = AllData.priceMapBar.get(symbol).lastEntry().getValue().getClose();
//                            } else {
...
//                        , ConcurrentSkipListMap::new
//                        , Collectors.summingDouble(e -> e.getValue()
//                                .getPnl(symb, AllData.priceMap.getOrDefault(symb, 0.0)))),
//                        e -> e.entrySet().stream().sorted(reverseComp(Comparator.comparing(Entry::getValue)))
//                                .map(e1 -> str("|||", e1.getKey(),
//                                        "#:", numTradesByOrder.getOrDefault(e1.getKey(), 0L),
//                                        "Tot Q: ", quantitySumByOrder.getOrDefault(e1.getKey(), 0d), r(e1.getValue())))
...
//            for (Entry<LocalTime, TradeBlock> e : trMap.headMap(prMap.firstKey(), false).entrySet()) {
...
//    }
//
//    static synchronized void mtmPnlCompute(Predicate<? super Entry<String, ?>> p, String nam) {
//        //pr(" COMPUTE *************************************************** ");
//        gPnl.setName(nam);
//        gPnl.setChineseName(nameMap.getOrDefault(nam, ""));
//
//        if (AllData.priceMap.getOrDefault(nam, 0.0) == 0.0 && AllData.priceMapBar.containsKey(nam) && AllData.priceMapBar.get(nam).size() > 0) {
//            AllData.priceMap.put(nam, Optional.ofNullable(AllData.priceMapBar.get(nam).lastEntry())
...
//                                                    * getNetPosition(s.getKey()) * AllData.priceMap.getOrDefault(s.getKey(), 0.0)))))
...
//        } else if (AllData.priceMapBar.containsKey(symbol) && AllData.priceMapBar.get(symbol).size() > 0) {
//            return AllData.priceMapBar.get(symbol)
//                    .entrySet().stream()
//                    .filter(e -> e.getKey().isAfter(ltof(8, 59)))
//                    .findFirst().map(Entry::getValue).map(SimpleBar::getOpen).orElse(0.0);
//        } else if (AllData.priceMapBarDetail.containsKey(symbol) && AllData.priceMapBarDetail.get(symbol).size() > 0) {
//            return AllData.priceMapBarDetail.get(symbol)
//                    .entrySet().stream()
...
//        //if(priceMapBar.)
//        double fx = fxMap.getOrDefault(currencyMap.getOrDefault(name, CNY), 1.0);
//        double defaultPrice = Optional.ofNullable(AllData.priceMapBar.get(name).lastEntry())
//                .map(Entry::getValue).map(SimpleBar::getClose).orElse(0.0);
...
//                .sorted((Comparator.comparingDouble((ToDoubleFunction<Entry<String, Double>>) Entry::getValue).reversed()))
...
//            LocalTime lastKey = AllData.priceMapBar.get(name).lastKey().isAfter(LocalTime.of(15, 0))
...
//        double minT = 0.0;
//        if (AllData.priceMapBar.containsKey(name) && AllData.priceMapBar.get(name).size() > 0) {
//            curr = AllData.priceMapBar.get(name).lastEntry().getValue().getClose();
//            maxT = AllData.priceMapBar.get(name).entrySet().stream().max(BAR_HIGH).map(Entry::getValue)
//                    .map(SimpleBar::getHigh).orElse(0.0);
//            minT = AllData.priceMapBar.get(name).entrySet().stream().min(BAR_LOW).map(Entry::getValue)
//                    .map(SimpleBar::getHigh).orElse(0.0);
//        }
...
//        if (AllData.priceMapBar.containsKey(name) && AllData.priceMapBar.get(name).size() > 0) {
//            curr = AllData.priceMapBar.get(name).lastEntry().getValue().getClose();
//            maxT = reduceMapToDouble(AllData.priceMapBar.get(name), SimpleBar::getHigh, Math::max);
//            minT = reduceMapToDouble(AllData.priceMapBar.get(name), SimpleBar::getLow, Math::min);
//        }
//
//        double max = Math.max(maxT, wtdMaxMap.getOrDefault(name, 0.0));
//        double min = Math.min(minT, wtdMinMap.getOrDefault(name, 0.0));
...
//                * AllData.priceMap.getOrDefault(name, 0.0) * getNetPosition(name) / 1000d);
//    }
//
...
//
//        Map<String, Double> tickerNetpnl = AllData.priceMapBar.entrySet().stream().filter(e -> relevantStock(e.getKey()))
//                .collect(Collectors.toMap(Entry::getKey, e -> getTodayTotalPnl(e.getKey())));
//
//        double netPnlAll = tickerNetpnl.entrySet().stream().mapToDouble(Entry::getValue).sum();
//
//        res = tickerNetpnl.entrySet().stream().sorted((Comparator.comparingDouble((Entry<String, Double> e) -> Math.abs(e.getValue())).reversed()))
//                .map(Entry::getKey).map(s -> Utility.str(nameMap.get(s),
...
//                defaultPrice = AllData.priceMapBarDetail.get(symbol).lastEntry().getValue();
//            }
//
//            double currPrice = AllData.priceMap.getOrDefault(symbol, 0.0) == 0.0 ?
//                    ChinaStock.closeMap.getOrDefault(symbol, defaultPrice) : AllData.priceMap.get(symbol);
//
//            double priceNow = currPrice;
//            if (AllData.priceMap.getOrDefault(symbol, 0.0) != 0.0) {
//                priceNow = AllData.priceMap.get(symbol);
...
//                case 22:
//                    if (AllData.priceMapBarDetail.containsKey(symbol) &&
//                            AllData.priceMapBarDetail.get(symbol).size() > 0) {
//                        return AllData.priceMapBarDetail.get(symbol).firstEntry().getValue();
//                    } else {
//                        return 0.0;
//                    }
//                case 23:
//                    if (AllData.priceMapBarDetail.containsKey(symbol) &&
//                            AllData.priceMapBarDetail.get(symbol).size() > 0) {
//                        return AllData.priceMapBarDetail.get(symbol).firstEntry().getKey().toLocalDate();
...
//                    if (AllData.priceMapBarDetail.containsKey(symbol) &&
//                            AllData.priceMapBarDetail.get(symbol).size() > 0) {
//                        return Math.round(10000d * (AllData.priceMapBarDetail.get(symbol).lastEntry().getValue() /
//                                AllData.priceMapBarDetail.get(symbol).firstEntry().getValue() - 1)) / 100d;
//                    }
//                    return 0.0;
//                case 25:
//                    if (ytdData.containsKey(symbol) && ytdData.get(symbol).size() > 0
...
//                    AllData.priceMapBar.get("SGXA50PR").put(lt, new SimpleBar(open, high, low, close));
//                }
//            }
//
